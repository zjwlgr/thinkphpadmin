$(function(){
    ...
    $("#form1_login").bind("submit", function(){
        ...
        $.post("/adminindex/Login/login_action.html",$("#form1_login").serialize(),function(data){
            if(data.code == 0){
                art.dialog({lock: true,opacity: 0.5,content: data.msg,icon: 'error',ok: function(){
                    if(data.token == 'no'){window.location.reload();}
                }});
                $(".image").click();
                $("input[name=__hash__]").val(data.token);
